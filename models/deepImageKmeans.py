...
class DeepKmeans:
    def __init__(self, n_clusters=10):
        self.model = MiniBatchKMeans(n_clusters=n_clusters, random_state=0, init_size=1024, reassignment_ratio=0.1)
        self.pca = IncrementalPCA(n_components=2,whiten=False)
        self.kpca = KernelPCA(n_components=2,kernel="rbf",alpha=2.67,gamma=5e-4,n_jobs=8)
        self.tsvd = TruncatedSVD(n_components=1)
        self.tsne = TSNE(n_components=2,perplexity=23,init="pca")
    ...
    def train(self, df: DataFrame, km : MiniBatchKMeans, path) -> List:
        ...
        accuracy = self.classification_accuracy(predicted_cluster, y)
        loss = km.inertia_
    ...
    def predict(self, df: DataFrame, km: MiniBatchKMeans, path) -> List:
        with open(path, "rb") as f:
            features = np.load(f)
